import React from 'react'
import { cn } from '../../lib/utils'
import { motion } from 'framer-motion'
import { Container } from '../layout/Container'
import { Button } from '../ui/Button'
import { GlowOrb } from '../ui/GlowOrb'

interface PersonalHeroProps {
  name: string
  greeting?: string
  tagline: string
  imageUrl?: string
  socialLinks?: {
    github?: string
    linkedin?: string
    twitter?: string
    email?: string
  }
  className?: string
}

export const PersonalHero: React.FC<PersonalHeroProps> = ({
  name,
  greeting = "Hi, I'm",
  tagline,
  imageUrl,
  socialLinks,
  className,
}) => {
  return (
    <section className={cn('relative min-h-screen flex items-center overflow-hidden', className)}>
      {/* Background Effects */}
      <div className="absolute inset-0 -z-10">
        <GlowOrb size="xl" color="purple" className="top-20 -left-48" />
        <GlowOrb size="lg" color="pink" className="bottom-20 -right-32" />
        <GlowOrb size="md" color="cyan" className="top-1/3 right-1/3" animate />
      </div>

      {/* Grid Background */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#4f4f4f12_1px,transparent_1px),linear-gradient(to_bottom,#4f4f4f12_1px,transparent_1px)] bg-[size:24px_24px]" />
      </div>

      <Container className="relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16">
          {/* Image Section */}
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.5 }}
            className="flex-shrink-0"
          >
            <div className="relative">
              {imageUrl ? (
                <img
                  src={imageUrl}
                  alt={name}
                  className="w-64 h-64 lg:w-80 lg:h-80 rounded-full object-cover border-4 border-accent-purple/20"
                />
              ) : (
                <div className="w-64 h-64 lg:w-80 lg:h-80 rounded-full bg-gradient-to-br from-accent-purple to-accent-pink flex items-center justify-center">
                  <span className="text-6xl lg:text-7xl font-bold text-white">
                    {name.charAt(0)}
                  </span>
                </div>
              )}
              {/* Glow effect around image */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-accent-purple/30 to-accent-pink/30 blur-2xl -z-10" />
            </div>
          </motion.div>

          {/* Content Section */}
          <div className="flex-1 text-center lg:text-left">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.2 }}
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
                <span className="text-text-secondary">{greeting}</span>{' '}
                <span className="gradient-text">{name}</span>{' '}
                <motion.span
                  initial={{ opacity: 0, rotate: -20 }}
                  animate={{ opacity: 1, rotate: 0 }}
                  transition={{ duration: 0.5, delay: 0.5 }}
                  className="inline-block origin-bottom-right"
                >
                  👋
                </motion.span>
              </h1>
            </motion.div>

            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              className="text-xl md:text-2xl text-text-secondary mb-8 max-w-2xl"
            >
              {tagline}
            </motion.p>

            {/* Social Links */}
            {socialLinks && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.4 }}
                className="flex flex-wrap gap-4 justify-center lg:justify-start"
              >
                {socialLinks.github && (
                  <Button
                    variant="glass"
                    size="md"
                    onClick={() => window.open(socialLinks.github, '_blank')}
                  >
                    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                    </svg>
                    GitHub
                  </Button>
                )}
                {socialLinks.linkedin && (
                  <Button
                    variant="glass"
                    size="md"
                    onClick={() => window.open(socialLinks.linkedin, '_blank')}
                  >
                    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                    </svg>
                    LinkedIn
                  </Button>
                )}
                {socialLinks.twitter && (
                  <Button
                    variant="glass"
                    size="md"
                    onClick={() => window.open(socialLinks.twitter, '_blank')}
                  >
                    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                    </svg>
                    Twitter
                  </Button>
                )}
                {socialLinks.email && (
                  <Button
                    variant="glass"
                    size="md"
                    onClick={() => window.location.href = `mailto:${socialLinks.email}`}
                  >
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                    Email
                  </Button>
                )}
              </motion.div>
            )}
          </div>
        </div>
      </Container>
    </section>
  )
}